...
void				move_shape(char **matrix, int get_x_offset,
						int get_y_offset);
void				figure_offset(char **matrix);
int					is_fit(char	**matrix, char **squard_to_fill, int x, int y);
int					multiple_fit(char **squard_to_fill, figure *f,
						int fig_counter, int squard_size);
void				del_me(char **squard_to_fill, int nb_of_fig_to_del,
						int squard_size);
void				ok_here(char **squard_to_fill, figure *f,
						int y, int x, int fig_counter);
...
int					where_is_xy(char **squard_to_fill, figure *f,
						int fig_counter, int squard_size);
int					can_i_shift(char **squard_to_fill, figure *f,
						int fig_counter, int squard_size);
int					control_fn(int fig_number, char **squard_to_fill,
						int squard_size, figure **array_of_figures);
int					is_full(char **squard_to_fill, int squard_size);

#endif

// Norme: ./fillit.h
// Error (line 22, col 8): shape is not well prefixed. Must be s_shape
// Error (line 27, col 6): figure have not a good prefix. Must be t_figure